...
// Validations explicitly matched with SupplyChain Mongoose properties
const schema = yup.object({
  ...
  requiredLoanAmount: yup
    .number()
    .typeError("Enter a valid number")
    .positive("Amount must be greater than 0")
    .required("Loan amount is required"),
  propertytype: yup.string().trim().required("Select business funding asset type"),
  marketvalue: yup.string().trim().required("Annual revenue turnover range is required"),
...
export default function InvestFormSupplyChain({
  ...
  const handleSubmit = async (e) => {
    ...
    schema
      ...
      .then(async () => {
        ...
        console.log("API URL:", API_CONFIG.LOANS.SUPPLY_CHAIN);
        console.log("Request Body:", {
          ...formData,
          productCategory: "Supply Chain Finance",
        });

        // Utilizing your streamlined apiCall helper mapping 
        const response = await apiCall(API_CONFIG.LOANS.SUPPLY_CHAIN, {
          method: "POST",
          body: JSON.stringify({
            ...formData,
            productCategory: "Supply Chain Finance",
        ...
        if (response.success) {
          ...
          alert(response.error || "Failed to submit financing request.");
        }
      })
      ...
      {open && typeof document !== "undefined" && createPortal(
        ...
        >
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-label="Apply for Supply Chain Finance"
          >
            <button
              onClick={handleClose}
              aria-label="Close"
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                ...
                <div style={row2}>
                  <Field
                    ...
                    placeholder="Required Loan Amount (INR)"
                    value={formData.requiredLoanAmount}
                    onChange={handleChange}
                    onBlur={handleBlur}
                    error={errors.requiredLoanAmount}
                    touched={touched.requiredLoanAmount}
                  />
                  <Field
                    as="select"
                    name="employeeType"
                ...
                <div style={row2}>
                  ...
                  >
                    <option value="" disabled>Business Financing Asset Option</option>
                    <option value="Unpaid Accounts Receivables">Unpaid Invoices / Bill Discounts</option>
                    <option value="Confirmed Purchase Orders">Purchase Order Funding</option>
                    <option value="Inventory Procurement">Working Capital Stock</option>
                    <option value="Vendor Allocation Plan">Vendor / Dealer Allocation</option>
                  </Field>
                  <Field
                    name="marketvalue"
                    placeholder="Annual Business Turnover (e.g. 1Cr - 50Cr)"
              ...
              <p style={{ fontSize: "12px", marginTop: "6px", color: "#334155" }}>
                Your financial request transactions are entirely protected by enterprise system safeguards.
              </p>
