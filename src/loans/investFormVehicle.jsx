...
const schema = yup.object({
  ...
  requiredLoanAmount: yup.string().trim(),
  income: yup.string().trim().required("Income is required"),
  amountrange: yup.string().trim(),
  loantype: yup.string().trim().required('Please select car type'),
...
const initialFormData = {
  ...
  amountrange: '',
  loantype: '',
  productCategory: 'vehicle-loan',
  agree: false
}

console.log('API_CONFIG:', API_CONFIG)

export default function InvestFormVehicle({
  buttonLabel = 'Apply Now',
  buttonClassName = 'bg-[#8bc700] text-white border-none py-2 px-4 rounded-md font-bold cursor-pointer transition-colors hover:bg-[#7ab600]'
}) {
  ...
  const handleSubmit = e => {
    ...
    schema
      ...
      .then(async () => {
        setErrors({})
        try {
          const response = await fetch(API_CONFIG.FORMS.SUBMIT_LOAN_FORM('vehicle-loan'), {
            method: 'POST',
            headers: {
  ...
  return (
    <div>
      ...
      {open && typeof document !== 'undefined' && createPortal(
        ...
        >
          <div
            className="bg-[#f5f8fc] text-[#1f2933] rounded-xl w-full max-w-[860px] border border-[#d9e2ec] shadow-[0_18px_48px_rgba(0,0,0,0.14)] p-5 relative max-h-[calc(100vh-24px)] overflow-y-auto"
            onClick={e => e.stopPropagation()}
            role='dialog'
            aria-modal='true'
            aria-label='Apply For Vehicle Loans'
          >
            <button
              onClick={handleClose}
              aria-label='Close'
              className="absolute top-2.5 right-3 border-none bg-transparent text-lg font-bold cursor-pointer text-[#64748b] hover:text-[#1f2933]"
            >
              ×
            </button>

            <h2 className="text-center m-0 mb-4 text-[#7ab600] text-2xl font-bold">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-2.5">
              ...
              <div className="text-center mt-3">
                <button
                  type='submit'
                  className="bg-gradient-to-r from-[#8bc700] to-[#7ab600] text-white border-none py-1.5 px-3 rounded-md font-normal text-sm cursor-pointer min-w-[100px] shadow-[0_8px_15px_rgba(139,199,0,0.3)] transition-transform active:scale-95"
                >
                  Apply Now
                </button>
              </div>
...
function Field({ as = 'input', error, touched, className = '', ...props }) {
  ...
  return (
    <div className="flex flex-col gap-0.5 w-full">
      <Component
        {...props}
        className={`w-full h-9 px-2.5 py-1.5 leading-5 box-border rounded-md text-[12.5px] bg-white text-gray-800 shadow-[inset_0_1px_2px_rgba(0,0,0,0.04)] focus:outline-none focus:ring-1 focus:ring-[#8bc700] transition-all
          ${touched && error ? 'border border-amber-500' : 'border border-[#cbd5e1]'} 
          ${className}`}
        aria-invalid={touched && Boolean(error)}
      />
      <div className="min-h-[16px]">
        {touched && error ? (
          <div className="text-amber-600 text-[11.5px] mt-1 pl-0.5">{error}</div>
        ) : null}
      </div>
    </div>
  )
