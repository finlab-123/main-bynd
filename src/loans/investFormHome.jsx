...
// Base schema used for global entries
const baseSchema = {
  ...
  requiredAmount: yup.string().trim(),
  income: yup.string().trim().required("Income is required"),
  marketValue: yup.string().trim(),
...
export default function InvestForm({
  ...
  buttonClassName,
  buttonStyle,
  currentPath = "home", // 💡 Added prop to dynamically send data to specific backend mappings
}) {
  ...
  // 💡 Dynamically check if the page requires property inputs to bypass global block validation crashes
  const dynamicSchema = yup.object({
    ...baseSchema,
    propertyDescription: currentPath === "home" || currentPath === "loan-against-property"
      ? yup.string().trim().required("Select property type")
      : yup.string().trim(),
  });
  ...
  const handleSubmit = (e) => {
    ...
    dynamicSchema
      ...
      .then(async () => {
        setErrors({});
        try {
          // 💡 FIXED: Uses centralized API_CONFIG for proper endpoint routing
          const apiUrl = API_CONFIG.FORMS.SUBMIT_LOAN_FORM(currentPath);
          const response = await fetch(apiUrl, {
              ...
              body: JSON.stringify({
                ...formData,
                loanAmount: formData.requiredAmount ? Number(formData.requiredAmount) : 0,
              }),
            }
          );
          if (!response.ok) {
  ...
  // Clean title header strings based on selection
  const formTitle = currentPath
    .split("-")
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                ...
                <div style={row2}>
                  ...
                  >
                    <option value="" disabled>employeeType Type</option>
                ...
                {/* 💡 Conditionally render property selections so it handles loans vs general investments smoothly */}
                {(currentPath === "home" || currentPath === "loan-against-property") && (
                  <div style={row2}>
                    <Field
                      as="select"
                    ...
                    >
                      ...
                      <option>Self-Owned Residential</option>
                      <option>Ready-to-Move Residential</option>
                    </Field>
                    <Field
                      name="marketValue"
              ...
              <label style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "12px", marginTop: "14px" }}>
                <input
                  type="checkbox"
                  name="agree"
                ...
                />
                <span>
                  I agree to the <a href='/terms' style={{ color: '#0d6efd' }}>Terms & Conditions</a> and <a href='/privacy-policy' style={{ color: '#0d6efd' }}>Privacy Policy</a>.
                </span>
              </label>
